...
class DatabaseQueries:
    ...
    def create_product(self, product_id: int, brand_id: int, name: str) -> bool:
            """Create a new product with minimal required fields."""
            query = """
                INSERT INTO Product (ProductID, BrandID, Name)
                VALUES (?, ?, ?)
            """
            try:
                with self.db.get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(query, (product_id, brand_id, name))
                    conn.commit()
                    return True
            except Exception as e:
