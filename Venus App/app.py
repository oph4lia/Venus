...
def delete_product_endpoint(product_id, brand_id):
    try:
        result = db.delete_product(product_id, brand_id)
        if not result:
...
def get_sorted_products():
    ...
    print(f"Received sort parameter: {sort_by}")  # Debug print

    try:
        if sort_by == "price_asc":
            products = db.get_products_sorted_by_price_asc()
            print("Sorting by price (asc)")  # Debug print
        elif sort_by == "price_desc":
            products = db.get_products_sorted_by_price_desc()
            print("Sorting by price (desc)")  # Debug print
        elif sort_by == "rating":
            products = db.get_products_sorted_by_rating()
            print("Sorting by rating")  # Debug print
        else:
            products = db.get_products_sorted_by_default()
            print("Default sorting")  # Debug print

        # Print first few products to verify sorting
        print("First few products:", products[:3])
